...
class MPControl():
    ...
    def run_mpc(self, x0, xr):
        ...
        chi_dot = self.auv.compute_nonlinear_dynamics(x0, u, complete_model=True)
        
        chi_dot = np.array(evalf(chi_dot))

        chi_sim = x0 + chi_dot[0:12, :] * self.dt
        chi_sim[3:6, :] = self.wrap_pi2negpi(chi_sim[3:6, :])
        
        self.path_length += np.linalg.norm(chi_sim[0:3, 0] - x0[0:3, 0])
        self.distance = np.linalg.norm(chi_sim[0:12, :] - xr[0:12, :])
        
        self.wec_data["state"]["eta"][:, self.time_id] = xr[0:6, :].flatten()
        self.wec_data["state"]["nu_r"][:, self.time_id] = xr[6:12, :].flatten()
        
        self.nav_data["state"]["eta"][:, self.time_id+1] = chi_sim[0:6, :].flatten()
        self.nav_data["state"]["nu_r"][:, self.time_id+1] = chi_sim[6:12, :].flatten()
        self.nav_data["control"]["u"][:, self.time_id] = u.flatten()
        self.nav_data["analysis"]["eta_dot"][:, self.time_id] = chi_dot[0:6, :].flatten()
        self.nav_data["analysis"]["nu_r_dot"][:, self.time_id] = chi_dot[6:12, :].flatten()
        ...
        print("----------------------------------------------")
        print(f"True Vehicle Pose: {np.round(chi_sim[0:6], 3).T}")
        print(f"True Vehicle Velocity: {np.round(chi_sim[6:12], 3).T}")
        print("----------------------------------------------")
