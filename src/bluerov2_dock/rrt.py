...
class RRT:
    ...
    def __init__(self, start, goal):
        ...
        self.print_verbose()
        self.bounds = np.array([-50.0, -50.0])
        # self.min_rand = 0
        # self.max_rand = 1000
        self.step_size = 0.5
    ...
    def print_verbose(self):
        print('Start: {}'.format(self.start_state))
        print('Goal: {}'.format(self.goal_state))
    ...
    def check_hit(self, start, goal):
        ...
        # Went off the maze
        if (x < self.bounds[0]) or (y < self.bounds[0]) or (x >= self.bounds[1]) or (y >= self.bounds[1]):
            return True
    ...
    def get_random_node(self):
        if np.random.rand() > self.goal_sample_rate:
            rand = self.Node(random.uniform(self.bounds[0], self.bounds[1]),
